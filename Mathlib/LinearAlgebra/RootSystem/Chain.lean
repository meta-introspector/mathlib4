...
include h in
lemma chainBotCoeff_add_chainTopCoeff_eq_pairingIn_chainTopIdx [P.IsReduced] :
...
lemma chainBotCoeff_add_chainTopCoeff_le_two [P.IsReduced] [P.IsNotG2] :
...
lemma chainBotCoeff_mul_chainTopCoeff [P.IsReduced] [P.IsIrreducible] [P.IsNotG2]
  ...
  have hki : P.pairingIn ℤ k i ≤ 0 ∧
    ...
    /- Consider the `P.root i`-chain through `P.root k`. -/
    have aux₁ : LinearIndependent R ![P.root i, P.root k] :=
      P.linInd_of_add_mem_range_root <| by rw [add_comm, h₁]; exact mem_range_self l
    rw [← P.chainBotCoeff_sub_chainTopCoeff aux₁]
    have aux₂ := P.chainBotCoeff_add_chainTopCoeff_le_two (i := i) (j := k)
    have aux₃ : 1 ≤ P.chainTopCoeff i k := by
      rw [← root_add_nsmul_mem_range_iff_le_chainTopCoeff aux₁, one_smul, h₁]
      exact mem_range_self l
    rcases eq_or_ne (P.chainBotCoeff i k) (P.chainTopCoeff i k) with aux₄ | aux₄ <;>
  ...
  · /- Geck "Case 2" -/
    replace hkj : P.chainTopCoeff j k = 1 := by simpa [hkj'] using hkj
    -- Factors through "Case 1" argument (with some roles changed)
    sorry
  /- Geck "Case 3" -/
  ...
  have aux₁ : ¬ (P.chainBotCoeff i m = 1 ∧ P.chainTopCoeff j l = 0) := by
    ...
    have aux₀ : P.pairingIn ℤ j i = P.pairingIn ℤ k i := by
      -- Trivial using `hmi'`
      sorry
    have aux₁ : P.pairingIn ℤ j i < 0 := by sorry
    ...
    let r (i : ι) : span ℤ (range P.root) := ⟨P.root i, subset_span <| mem_range_self i⟩
    have aux₃ : B.posForm (r i) (r i) ≤ B.posForm (r j) (r j) ∧
        B.posForm (r j) (r j) < B.posForm (r k) (r k) := by
      sorry
    have aux₄ : B.posForm (r i) (r i) = B.posForm (r j) (r j) := by
      -- easy but (upstream API needs work)
      have := B.toInvariantForm.apply_eq_or_of_apply_ne (i := j) (j := k) (by sorry) i
      sorry
    have aux₅ : P.pairingIn ℤ k i = -1 := by
      replace aux₄ : B.toInvariantForm.form (P.root i) (P.root i) =
          B.toInvariantForm.form (P.root j) (P.root j) := sorry -- trivial
      have : P.pairingIn ℤ i j = -1 ∧ P.pairingIn ℤ j i = -1 := by
        have := P.pairingIn_pairingIn_mem_set_of_length_eq_of_ne aux₄ hij (by /- trivial -/ sorry)
        aesop
      omega
    have aux₆ : B.posForm (r k) (r k) ≤ B.posForm (r i) (r i) := by
      -- easy from `aux₅` but (upstream API needs work)
      sorry
    omega
  have aux₂ : ¬ (P.chainBotCoeff i m = 0 ∧ P.chainTopCoeff j l = 1) := by
    -- Factors through first half of "Case 3" directly above
    sorry
